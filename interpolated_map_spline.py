...
def interpolate(knot_xs, knot_ys, x_vals):

    # Normalize x_vals to the index space of knot_xs
    x_min, x_max = knot_xs[0], knot_xs[-1]
    x_vals_normalized = (x_vals - x_min) / (x_max - x_min) * (len(knot_xs) - 1)
    
    interpolated_values = _map_coordinates(knot_ys, [x_vals_normalized], order=3, mode='nearest', cval=0.0)
    
    return interpolated_values


# Louis Desdoigts Interpolation Code
