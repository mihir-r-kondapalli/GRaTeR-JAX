from regression import log_likelihood_1d_pos_all_pars_spline, log_likelihood_1d_full_opt
from disk_utils_jax import jax_model_all_1d, jax_model_all_1d_cent
...
def quick_optimize(target_image, err_map, flux_scaling=1e6, knots=jnp.linspace(1, -1, 6), disp = True, method = None,
                    iters = 500, **kwargs):
    ...
    soln = minimize(llp, init_guess, options=opt, method=method)
    if(disp):
        print(soln)
    return soln.x
...
# 10 onwards is spline parameters, pxInArcsec and distance are good kwargs to include
def quick_optimize_full_opt(target_image, err_map, flux_scaling=1e6, knots=jnp.linspace(1, -1, 6), disp = True, method = None,
    ...
    ksi_gamma_beta_guess = jnp.array([3, 2, 1])
    init_guess = jnp.concatenate([init_disk_guess, init_cent_guess, ksi_gamma_beta_guess, init_knot_guess])
...
def quick_image_full_opt(pars, flux_scaling=1e6, **kwargs):
    return jax_model_all_1d_cent(DustEllipticalDistribution2PowerLaws, InterpolatedUnivariateSpline_SPF, pars[0:5],
                                InterpolatedUnivariateSpline_SPF.pack_pars(pars[10:]), flux_scaling,
                                xc=pars[5], yc=pars[6], ksi0=pars[7], gamma=pars[8], beta=pars[9],**kwargs)