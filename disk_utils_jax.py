...
# Jax Model but with spline model implemented
@partial(jax.jit, static_argnums=(0,1))
def jax_model_spline(DistrModel, spline_model, disk_params, halfNbSlices=25, ksi0=3., gamma=2., beta=1., dstar=111.61,
                  nx=140, ny=140, pixel_scale=0.063, n_nodes=6, pxInArcsec=0.01225, distance=50.):
    ...
    scattered_light_image = ScatteredLightDisk.compute_scattered_light_jax_spline(disk_params_jax, distr_params, DistrModel, spline_model,
                                                                  x_vector, y_vector, scattered_light_map, image, limage, tmp,
                                                                  halfNbSlices)
...
@partial(jax.jit, static_argnums=(0,1))
def jax_model_spline_1d(DistrModel, spline_model, disk_params, flux_scaling, halfNbSlices=25, ksi0=3., gamma=2., beta=1.,
                 dstar=111.61, nx=140, ny=140, pixel_scale=0.063, n_nodes=6, pxInArcsec=0.01225, distance=50.):
    ...
    scattered_light_image = ScatteredLightDisk.compute_scattered_light_jax_spline(disk_params_jax, distr_params, DistrModel, spline_model, x_vector, y_vector,
                                                                           scattered_light_map, image, limage, tmp, halfNbSlices)
