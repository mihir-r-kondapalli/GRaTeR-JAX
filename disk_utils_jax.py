...
def jax_model_psf(DistrModel, FuncModel, disk_params, spf_params, psf_params, PSFModel = GAUSSIAN_PSF,
                  halfNbSlices=25, ksi0=3., gamma=2., beta=1., dstar=111.61,
                  nx=140, ny=140, pixel_scale=0.063, n_nodes=6, pxInArcsec=0.01225, distance=50.):
    ...
    psf_image = PSFModel.generate(jnp.meshgrid(x_vector, y_vector), PSFModel.init(psf_params))
    disk_image_psf = jss.convolve2d(scattered_light_image, psf_image)

    return disk_params['flux_scaling']*disk_image_psf