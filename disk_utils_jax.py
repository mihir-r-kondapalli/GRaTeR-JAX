...
@partial(jax.jit, static_argnums=(0,1))
def jax_model_scal(DistrModel, FuncModel, disk_params, spf_params,
    ...
    yc = jnp.where(ny%2==1, ny/2-0.5, ny/2)
    ...
    scattered_light_image = ScatteredLightDisk.compute_scattered_light_jax(disk_params_jax, distr_params, DistrModel, func_params,
...
# Scalar function using array for disk_params
# 0: alpha_in, 1: alpha_out, 2: sma, 3: inclination, 4: position_angle
@partial(jax.jit, static_argnums=(0,1))
def jax_model_scal_1d(DistrModel, FuncModel, disk_params, spf_params, flux_scaling,
                    halfNbSlices=25, ksi0=3., gamma=2., beta=1., dstar=111.61,
                    nx=140, ny=140, pixel_scale=0.063, n_nodes=6, pxInArcsec=0.01225, distance=50.):
