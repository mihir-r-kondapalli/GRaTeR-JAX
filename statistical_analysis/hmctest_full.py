...
from statistical_analysis.regression import log_likelihood_1d_full_opt, log_likelihood_1d_pos_cent
...
def run_hmc_ab(soln, target_image, err_map, row, name, nwalkers=250, niter=250, burns=70):

    # 0: alpha_in, 1: alpha_out, 2: sma, 3: inclination, 4: position_angle, 5: xc, 6: yc, 7: e, 8: omega, 9 onwards is spf parameters
    soln = np.concatenate([soln, np.array[0, 3, 2, 1]])    # adding distribution parameters

    knots = get_inc_bounded_knots(row["Inclination"], row["Radius"], buffer = 0, num_knots=jnp.size(soln[7:]))

    llp = lambda x: -log_likelihood_1d_full_opt(jnp.concatenate([x[0:7], jnp.exp(x[7:])]), 
                        DustEllipticalDistribution2PowerLaws, InterpolatedUnivariateSpline_SPF, 
    ...
    mc_model = HMC_model(llp, BOUNDS)
    mc_model.run(init_soln, niter=niter, nwarmup=burns)
